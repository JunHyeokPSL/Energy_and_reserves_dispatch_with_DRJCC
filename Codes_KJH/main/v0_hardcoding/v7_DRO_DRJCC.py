...
s_obj = m.addVars(nScen, lb = -100000, ub = 100000, vtype = gp.GRB.CONTINUOUS, name='s_obj')
...
print("Define the objective variable")
...
for i in range(nUnits): 
    m.addConstr(x_var[i] == p[i], name= f"x_var[{i}] gen const")
    m.addConstr(x_var[i+ len(p)] == ru[i], name= f"x_var[{i}] ru const")
    m.addConstr(x_var[i+ len(p)+len(ru)] == rd[i], name= f"x_var[{i}] rd const")

eps = DRO_param['eps_joint_cvar']
...
z_c = m.addVars(nScen, lb = -1000000, ub = 0, name = 'z_c')
v_c = m.addVars(nICC, lb = 0, name = 'v_c')
gamma = m.addVar(lb = 0, name = 'gamma')

lhs_max = m.addMVar((nICC,nScen), lb = 0, name = 'lhs_max')
lhs_sum = m.addMVar((nICC,nScen), lb = -1000000, name = 'lhs_sum')

sum_zc = gp.quicksum(z_c[ss] for ss in range(nScen))

print("start drjcc")
for k in range(nICC):
    m.addConstr(rho * v_c[k] - eps*gamma <= 1/nScen * sum_zc, name=f"rho_v_c const{k}")
    ...
    for n in range(nScen):
        ...
        rhs = z_c[n] + gamma
        m.addConstr( lhs_sum[k,n] == lhs_x + lhs_u, name=f"lhs_sum{k}_{n}")
        m.addGenConstrMax(lhs_max[k,n], [lhs_sum[k,n], 0.0], name="max_assign")
        m.addConstr(rhs <= lhs_max[k,n], name = f"max_const{k}_{n}")
        
    if k % 10 == 0:
        print(f"iteration {k} of sum drjcc")
        
print("finish max constraint of drjcc")



lhs_CC = m.addMVar((nICC,nWT), lb=-1000000, ub = 1000000, vtype = gp.GRB.CONTINUOUS, name='lhs_CC') 
lhs_dual_CC = m.addMVar((nICC),name='dro_dual_norm_CC')
for k in range(nICC):
    for ww in range(nWT):
        m.addConstr(lhs_CC[k,ww] ==  ( gp.quicksum( A_g[k,gg] * Y[gg,ww] for gg in range(nUnits)) + A_w[k,ww]), name=f"lhs_const{k}_{ww}") 
    m.addGenConstrNorm( lhs_dual_CC[k], lhs_CC[k,:], GRB.INFINITY, "Const_norm_CC" )
    #rhs_sc = rho * N * lhs_dual_CC[k]
    lhs_sc = lhs_dual_CC[k]
    rhs_sc = v_c[k] # eps * N * t_c[k] - sum_sc 
    
    m.addConstr( lhs_sc <= rhs_sc, name= f"dual_const{k}") 
    
    if k % 10 == 0:
        print(f"iteration {k}")
    
    
print("Define the constraint")
