...
s_c = m.addMVar((nICC, nScen), lb = -10000)
lambda_c = m.addVars(nICC, lb = -10000)
tau_c = m.addVars(nICC, lb = -10000)
eps = DRO_param['eps_joint_cvar'] / 25
# Add Dual Norm Constraint for

lhs_CC = m.addVars(nWT, vtype = gp.GRB.CONTINUOUS, name='lhs_CC') 
lhs_dual_CC = m.addMVar((nICC),name='dro_dual_norm_CC')

for k in range(nICC):
    ld = gp.quicksum(rho*lambda_c[k] + 1/nScen*s_c[k,ss] for ss in range(nScen))
    m.addConstr( ld  <= 0 )
    for n in range(nScen):p
        m.addConstr(tau_c[k] <= s_c[k,n])
        
        lhs = 1/eps  * gp.quicksum( A_g[k,gg] * Y[gg,ww] * Wscen_xi[ww][n] for gg in range(nUnits) for ww in range(nWT))
        lhs +=  gp.quicksum( A_w[k,ww] * Wscen_xi[ww][n] for ww in range(nWT))
        lhs -= 1/eps * gp.quicksum( b_g[k,j] * x_var[j] + b_c[k] for j in range(len(x_var)))      
        lhs += (1- 1/eps)*tau_c[k] 
        rhs = s_c[k,n]
        m.addConstr(lhs <= rhs)
     
    for ww in range(nWT):
        m.addConstr(lhs_CC[ww] ==  gp.quicksum( 1/eps *A_g[k,gg] * Y[gg,ww] for gg in range(nUnits)) + A_w[k,ww])
        
    m.addGenConstrNorm( lhs_dual_CC[k], lhs_CC, GRB.INFINITY, "Const_norm_CC" )
    m.addConstr(lhs_dual_CC[k] <= lambda_c[k])
