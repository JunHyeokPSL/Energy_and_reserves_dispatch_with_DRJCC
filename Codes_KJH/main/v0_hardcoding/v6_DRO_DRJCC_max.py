...
Wscen_xi = Wscen_xi/2
ResCap = sys_dict['ResCap'] * 2
sys_dict['F'] = sys_dict['F'] * 1.0
...
lhs_max = m.addMVar((nICC,nScen), name = 'lhs_max')
lhs_sum = m.addMVar((nICC,nScen), name = 'lhs_sum')
for k in range(nICC):
    ...
    for n in range(nScen):
        ...
        m.addConstr( lhs_sum[k,n] == lhs_x + lhs_u)
        
        m.addGenConstrMax(lhs_max[k,n], [lhs_sum[k,n], 0.0], name="max_assign")
        
        rhs = t_c[k] - s_c[k,n]
        
        m.addConstr(lhs_max[k,n] >= rhs)
