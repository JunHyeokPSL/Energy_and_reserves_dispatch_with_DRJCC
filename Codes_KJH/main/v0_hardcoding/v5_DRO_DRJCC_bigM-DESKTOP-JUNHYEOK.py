...
for k in range(nICC):
    ...
    rhs_sc = gp.quicksum( eps * N * t_c[k] - s_c[k,ss] for ss in range(nScen)) 
    m.addConstr( rho * N * lhs_dual_CC[k] <= rhs_sc) 
...
dual_cc_rhs_Sol = np.zeros(nICC)
dual_cc_rhs_A_Sol = np.zeros(nICC)
dual_cc_rhs_B_Sol = np.zeros(nICC)
...
for k in range(nICC):
    ...
    dual_cc_rhs_B_Sol[k] = sum_s_c_Sol[k]
    max_dual_cc_Sol[k] = max(dual_cc_lhs_Sol[k,:])
    # cc_Const_Sol[k] = eps * lambda_c_Sol[k] * const
    
    dual_cc_rhs_A_Sol[k] = rho * N * t_c_Sol[k]
    dual_cc_rhs_Sol[k] = dual_cc_rhs_A_Sol[k] - dual_cc_rhs_B_Sol[k] 
