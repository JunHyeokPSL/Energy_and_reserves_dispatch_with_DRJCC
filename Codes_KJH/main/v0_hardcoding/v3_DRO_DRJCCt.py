...
"[ORL] Energy and reserve dispatch with distributionally robust joint chance
constraints"
...
s_c = m.addMVar((nICC, nScen), lb = -1000000, name='s_c')
lambda_c = m.addVars(nICC, lb = -1000000, ub = 10000000, name='lambda_c')
tau_c = m.addVars(nICC, lb = -1000000, name='tau_c')
...
for k in range(nICC):
    ld = rho*lambda_c[k] + 1/nScen * gp.quicksum( s_c[k,ss] for ss in range(nScen))
    m.addConstr( ld  <= 0 )
    for n in range(nScen):
        m.addConstr(tau_c[k] <= s_c[k,n])
        
        # A_g : B(j,:) == jcc{j,2}
        # A_w : C(j,:) == jcc{j,3}
        # b_g : A(j,:) == jcc{j,1}
        # b_c : b(j) == -jcc{j,4}
        lhs = gp.quicksum( A_g[k,gg] * Y[gg,ww] * Wscen_xi[ww][n] for gg in range(nUnits) for ww in range(nWT))
        lhs +=  gp.quicksum( A_w[k,ww] * Wscen_xi[ww][n] for ww in range(nWT))
        lhs -= gp.quicksum( b_g[k,j] * x_var[j] for j in range(len(x_var))) + b_c[k]      
        lhs += (eps - 1)*tau_c[k] 
        rhs = eps * s_c[k,n]
        m.addConstr(lhs <= rhs)
    ...
    m.addConstr(lhs_dual_CC[k] <= eps * lambda_c[k] * const)
...
dual_cc_Sol = np.zeros((nICC,nWT))
max_dual_cc_Sol = np.zeros(nICC)
cc_Const_Sol = np.zeros(nICC)
lambda_c_Sol = np.zeros(nICC)
s_c_Sol = np.zeros((nICC, nScen)) 
sum_s_c_Sol = np.zeros(nICC)
tau_c_Sol = np.zeros(nICC)
for k in range(nICC):
    lambda_c_Sol[k] = m.getVarByName(f"lambda_c[{k}]").X
    for ww in range(nWT): 
        for gg in range(nUnits):
            dual_cc_Sol[k,ww] += A_g[k,gg] * Y_Sol[gg,ww]
        dual_cc_Sol[k,ww] += A_w[k,ww]
        
    for nn in range(nScen):
        s_c_Sol[k,nn] = m.getVarByName(f"s_c[{k},{nn}]").X
    sum_s_c_Sol[k] = 1/nScen * sum(s_c_Sol[k,:])
    max_dual_cc_Sol[k] = max(dual_cc_Sol[k,:])
    cc_Const_Sol[k] = eps * lambda_c_Sol[k] * const
    tau_c_Sol[k] = m.getVarByName(f"tau_c[{k}]").X
# for k in range(nScen):
#     for i in range(nUnits):
