...
from rts_data import generate_sys, generate_matrix, generate_H_matrix
...
# eps = DRO_param['eps_joint_cvar'] / 25
# # Add Dual Norm Constraint for


# lhs_CC = m.addMVar((nICC,nWT), lb=-100000, ub = 100000, vtype = gp.GRB.CONTINUOUS, name='lhs_CC') 
# lhs_dual_CC = m.addMVar((nICC),name='dro_dual_norm_CC')

# for k in range(nICC):
#     ld = gp.quicksum(rho*lambda_c[k] + 1/nScen*s_c[k,ss] for ss in range(nScen))
#     m.addConstr( ld  <= 0 )
...
#         lhs -= gp.quicksum( b_g[k,j] * x_var[j] + b_c[k] for j in range(len(x_var)))      
#         lhs += (eps - 1)*tau_c[k] 
#         rhs = eps * s_c[k,n]
#         m.addConstr(lhs <= rhs)
...
#     m.addConstr(lhs_dual_CC[k] <= eps * lambda_c[k] * 32)
...
#     cc_Const_Sol[k] = eps * lambda_c_Sol[k]
# # for k in range(nScen):
# #     for i in range(nUnits):
